...
function create_fragment(ctx) {
	...
	return {
		...
		l(nodes) {
			...
			if (!section.ic) div0 = claim_template_element(div0, first_element_child(section), [], section);
			if (!div0.ic) input = claim_template_element(input, first_element_child(div0), [], div0);
			if (!div0.ic) h1 = claim_template_element(h1, next_element_sibling(input), [], div0);
			if (!h1.ic) t4 = claim_template_text(t4, first_child(h1), [], h1);
			if (!h1.ic) t5 = claim_template_text(t5, next_sibling(t4), [], h1);
			if (!h1.ic) t6 = claim_template_text(t6, next_sibling(t5), [], h1);
			if (!div0.ic) p = claim_template_element(p, next_element_sibling(h1), [], div0);
			if (!p.ic) t8 = claim_template_text(t8, first_child(p), [], p);
